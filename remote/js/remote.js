...
thisObject.isDown=true;var isValidKey=false;console.log("key code = "+event.keyCode+", ctrl = "+event.ctrlKey);var isCtrl=event.ctrlKey||event.metaKey;if(isCtrl==true)
...
break;case Key.ENTER:isValidKey=true;$("#selectButton").addClass("select_active");remote.sendRequest(RequestType.SELECT);break;case Key.BACKSPACE:isValidKey=true;remote.sendRequest(RequestType.BACK);break;case Key.ESCAPE:if(popout==1&&window["chrome"]&&window["chrome"].extension)
{remote.sendRequest(RequestType.BACK);}
break;case Key.STOP:case Key.STOP_DEP:isValidKey=true;remote.sendRequest(RequestType.STOP);break;case Key.MUTE:isValidKey=true;remote.sendRequest(RequestType.MUTE);break;case Key.MENU:isValidKey=true;remote.sendRequest(RequestType.HOME);break;case Key.OSD:isValidKey=true;remote.sendRequest(RequestType.SHOW_OSD);break;case Key.LEFT:isValidKey=true;thisObject.isDown=false;if(isCtrl)
...
break;case Key.Q:case Key.EQUALS:isValidKey=true;thisObject.isDown=false;remote.sendRequest(RequestType.VOLUME_UP);break;case Key.W:case Key.MINUS:isValidKey=true;thisObject.isDown=false;remote.sendRequest(RequestType.VOLUME_DOWN);break;case Key.SKIP_NEXT:isValidKey=true;remote.sendRequest(RequestType.SKIP_NEXT);break;case Key.SKIP_PREVIOUS:isValidKey=true;remote.sendRequest(RequestType.SKIP_PREVIOUS);break;}
if(isValidKey==true)
...
{thisObject.sendRequest(RequestType.SKIP_PREVIOUS);});thisObject.bindFastClick($("#nextTrack"),function(event)
{thisObject.sendRequest(RequestType.SKIP_NEXT);});thisObject.bindFastClick($("#mute"),function(event)
...
{thisObject.showSendTextPanel();});thisObject.bindFastClick($("#power"),function(event)
{thisObject.showShutDownPanel();});thisObject.bindFastClick($("#cancelButton-shutDownPanel"),function(event)
{thisObject.hideShutDownPanel();});thisObject.bindFastClick($("#restartButton-shutDownPanel"),function(event)
{thisObject.hideShutDownPanel();thisObject.sendRequest(RequestType.RESTART);});thisObject.bindFastClick($("#shutDownButton-shutDownPanel"),function(event)
{thisObject.hideShutDownPanel();thisObject.sendRequest(RequestType.SHUTDOWN);});thisObject.bindFastClick($("#quitButton-shutDownPanel"),function(event)
{thisObject.hideShutDownPanel();thisObject.sendRequest(RequestType.QUIT);});thisObject.bindFastClick($("#suspendButton-shutDownPanel"),function(event)
{thisObject.hideShutDownPanel();thisObject.sendRequest(RequestType.SUSPEND);});thisObject.bindFastClick($("#hibernateButton-shutDownPanel"),function(event)
{thisObject.hideShutDownPanel();thisObject.sendRequest(RequestType.HIBERNATE);});thisObject.bindFastClick($("#sendTextDataButton"),function(event)
{thisObject.hideSendPanel();var sendText=document.getElementById("sendTeatArea").value;var params={text:sendText,done:true};xbmcSocket.send("Input.SendText",params);});$('#sendTeatArea').keypress(function(event)
{var sendText=document.getElementById("sendTeatArea").value;var params;if(event.which==Key.ENTER)
{params={text:sendText,done:true};xbmcSocket.send("Input.SendText",params);return false;}});thisObject.bindFastClick($("#backDataButton"),function(event)
{thisObject.hideSendPanel();var sendText="";var params={text:sendText,done:true};xbmcSocket.send("Input.SendText",params);});thisObject.bindFastClick($("#sendPasswordDataButton"),function(event)
...
{var obj=JSON.parse(data);console.log("done");if(obj.result.length>0)
{for(var i=0;i<obj.result.length;i++)
{var params={playerid:obj.result[i].playerid};xbmcSocket.send("Player.PlayPause",params);}}});break;case RequestType.INFO:xbmcSocket.send("Input.Info");break;case RequestType.CONTEXT_MENU:xbmcSocket.send("Input.ContextMenu");break;case RequestType.SELECT:xbmcSocket.send("Input.Select");break;case RequestType.BACK:xbmcSocket.send("Input.Back");break;case RequestType.MUTE:params={action:"mute"};xbmcSocket.send("Input.ExecuteAction",params);break;case RequestType.HOME:xbmcSocket.send("Input.Home");break;case RequestType.SHOW_OSD:xbmcSocket.send("Input.ShowOSD");break;case RequestType.MOVE_LEFT:xbmcSocket.send("Input.Left");break;case RequestType.SEEK_BACK:params={action:"stepback"};xbmcSocket.send("Input.ExecuteAction",params);break;case RequestType.MOVE_RIGHT:xbmcSocket.send("Input.Right");break;case RequestType.SEEK_FRONT:params={action:"stepforward"};xbmcSocket.send("Input.ExecuteAction",params);break;case RequestType.MOVE_UP:xbmcSocket.send("Input.Up");break;case RequestType.MOVE_DOWN:xbmcSocket.send("Input.Down");break;case RequestType.VOLUME_UP:params={action:"volumeup"};xbmcSocket.send("Input.ExecuteAction",params);break;case RequestType.VOLUME_DOWN:params={action:"volumedown"};xbmcSocket.send("Input.ExecuteAction",params);break;case RequestType.SHUTDOWN:xbmcSocket.send("System.Shutdown");break;case RequestType.RESTART:xbmcSocket.send("System.Reboot");break;case RequestType.HIBERNATE:xbmcSocket.send("System.Hibernate");break;case RequestType.SUSPEND:xbmcSocket.send("System.Suspend");break;case RequestType.QUIT:xbmcSocket.send("Application.Quit");break;case RequestType.UPDATE_LIBRARY:xbmcSocket.send("VideoLibrary.Scan");break;case RequestType.SKIP_NEXT:params={action:"skipnext"};xbmcSocket.send("Input.ExecuteAction",params);break;case RequestType.SKIP_PREVIOUS:params={action:"skipprevious"};xbmcSocket.send("Input.ExecuteAction",params);break;}};this.showShutDownPanel=function()
{$("#main, #footer").fadeTo("fast",0.1).promise().done(function()
{$("#shutdown_panel").show();});};this.hideShutDownPanel=function()
{$("#shutdown_panel").hide();$("#main, #footer").fadeTo("fast",1);};this.showSendTextPanel=function(value)
{$("#main, #footer").fadeTo("fast",0.1).promise().done(function()
