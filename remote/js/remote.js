...
break;}};this.init=function()
{$("#info").click(function(event)
{xbmcSocket.send("Input.Info");event.preventDefault();});$("#osd").click(function(event)
{xbmcSocket.send("Input.ShowOSD");event.preventDefault();});$("#back").click(function(event)
{xbmcSocket.send("Input.Back");event.preventDefault();});$("#menu").click(function(event)
{xbmcSocket.send("Input.Home");event.preventDefault();});$("#selectButton").click(function(event)
{xbmcSocket.send("Input.Select");event.preventDefault();});$("#upArrow").click(function(event)
{xbmcSocket.send("Input.Up");event.preventDefault();});$("#downArrow").click(function(event)
{xbmcSocket.send("Input.Down");event.preventDefault();});$("#leftArrow").click(function(event)
{xbmcSocket.send("Input.Left");event.preventDefault();});$("#rightArrow").click(function(event)
{xbmcSocket.send("Input.Right");event.preventDefault();});$("#pause").click(function(event)
{var params={playerid:1};xbmcSocket.send("Player.PlayPause",params);event.preventDefault();});$("#stop").click(function(event)
{var params={playerid:1};xbmcSocket.send("Player.Stop",params);event.preventDefault();});$("#play").click(function(event)
{xbmcSocket.send("Player.GetActivePlayers",null,function(data)
{var obj=JSON.parse(data);console.log("done");if(obj.result.length>0)
{var params={playerid:1};xbmcSocket.send("Player.PlayPause",params);}
else
{console.log("just select");xbmcSocket.send("Input.Select");}});event.preventDefault();});$("#backward").click(function(event)
{var params={action:"stepback"};xbmcSocket.send("Input.ExecuteAction",params);event.preventDefault();});$("#forward").click(function(event)
{var params={action:"stepforward"};xbmcSocket.send("Input.ExecuteAction",params);event.preventDefault();});$("#prevTrack").click(function(event)
{var params={action:"skipprevious"};xbmcSocket.send("Input.ExecuteAction",params);event.preventDefault();});$("#nextTrack").click(function(event)
{var params={action:"skipnext"};xbmcSocket.send("Input.ExecuteAction",params);event.preventDefault();});$("#power").click(function(event)
{xbmcSocket.send("System.Shutdown");event.preventDefault();});$("#mute").click(function(event)
{params={action:"mute"};xbmcSocket.send("Input.ExecuteAction",params);event.preventDefault();});$("#context_menu").click(function(event)
{xbmcSocket.send("Input.ContextMenu");event.preventDefault();});$("#update_library").click(function(event)
{xbmcSocket.send("VideoLibrary.Scan");event.preventDefault();});$("#sendTextButton").click(function(event)
{thisObject.showSendTextPanel();event.preventDefault();});$("#backDataButton, #backPasswordDataButton").click(function(event)
{thisObject.hideSendPanel();params={action:"close"};xbmcSocket.send("Input.ExecuteAction",params);event.preventDefault();});$("#sendTextDataButton").click(function(event)
{thisObject.hideSendPanel();var sendText=document.getElementById("sendTeatArea").value;params={text:sendText,done:true};xbmcSocket.send("Input.SendText",params);event.preventDefault();});$("#sendPasswordDataButton").click(function(event)
{thisObject.hideSendPanel();var sendPassword=document.getElementById("sendPasswordInput").value;params={text:sendPassword,done:true};xbmcSocket.send("Input.SendText",params);event.preventDefault();});$("#power").addClass("power_on");$("#power").removeClass("power_off");window.onbeforeunload=thisObject.closeSocket;};this.showSendTextPanel=function(value)
...
{console.log("dispose");$("#info").off("click");$("#osd").off("click");$("#back").off("click");$("#menu").off("click");$("#selectButton").off("click");$("#upArrow").off("click");$("#downArrow").off("click");$("#leftArrow").off("click");$("#rightArrow").off("click");$("#pause").off("click");$("#stop").off("click");$("#play").off("click");$("#backward").off("click");$("#forward").off("click");$("#prevTrack").off("click");$("#nextTrack").off("click");$("#power").off("click");$("#mute").off("click");$("#context_menu").off("click");$("#update_library").off("click");$("#sendTextButton").off("click");$("#backDataButton").off("click");$("#sendTextDataButton").off("click");};this.closeSocket=function()
...
{document.addEventListener("touchend",function(event){document.activeElement.blur();},false);if(window["chrome"]&&window["chrome"].extension)
{console.log("chrome extension");background=chrome.extension.getBackgroundPage();}
else
{$("#settings").hide();}
$("#settings, #settings_new").click(function(event)
{window.location.href="settings.html?popout="+popout;event.preventDefault();});$("#popOut").hide();if(ALLOW_POPOUT)
